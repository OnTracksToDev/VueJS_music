...
const stock = ref(props.record.stock);

const incrementStock = () => {
  stock.value++;
  props.record.stock = stock.value;
};
...
<template>
  <div class="container px-5 mx-auto">
    ...
    >
      ...
      >
        <img
          :src="record.coverUrl ? record.coverUrl : '/src/assets/img/default.jpg'"
        />
      </div>
      <div class="flex-grow sm:text-left text-center mt-6 sm:mt-0">
        ...
        <h3 class="text-black text-xl title-font mb-2">
          {{ record.artist
          }}<span class="font-light mr-2">{{ record.year }}</span>
        </h3>
        <p class="leading-relaxed text-base">{{ record.comment }}</p>
        <div class="py-4">
          <div v-if="record.stock>0" class="inline-block mr-2">
          ...
          <div v-if="record.stock < 1"
              class="inline-block mr-2" >
            <!-- quand le stock est à zéro  -->
            <div class="flex pr-2 h-full items-center">
              <svg
                class="text-red-500 w-6 h-6 mr-1"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
              ...
              <p class="title-font font-medium text-red-500">out of stock</p>
            </div>
          </div>
        </div>
        <div class="md:flex font-bold text-gray-800">
          <div class="w-full md:w-1/2 flex space-x-3">
            <div class="w-1/2">
              <p>pitchfork pos : #{{ record.pitchforkPos }}</p>
              <!-- pitchfork pos  -->
            </div>
          </div>
          <div class="w-full">
